...
class CheckpointManager(object):
    ...
    def restore(self, filename):
        ...
        self.optim.load_stateDict(data['optim_state_dict'])
        self.scaler.load_stateDict(data['scaler_state_dict'])
        self.scheduler.load_stateDict(data['scheduler_state_dict'])
    ...
    for (batch_size, num_epochs, lr, momentum, weight_decay) in param_combinations:
        ...
        for epoch in range(ckpt.epoch + 1, num_epochs + 1):
            ...
            if epoch % args.val_period == 0:
                ...
                writers['val'].add_scalar('mAP', mAP, epoch)
                print(f"Epoch {epoch}/{num_epochs} - mAP: {mAP:.3f}")
