...
def main():
    ...
    # Lists to store losses and mAP for plotting
    train_losses = []
    val_losses = []
    mAPs = []
    ...
    for epoch in range(ckpt.epoch + 1, cfg.epochs + 1):
        ...
        # Store the train loss for plotting
        ...
        if epoch % args.val_period == 0:
            ...
            # Store the validation loss and mAP for plotting
            val_losses.append(metrics['loss'].result)
            mAPs.append(mAP)
    ...
    # Plot mAP
    plt.subplot(1, 2, 2)
    plt.plot(epochs, mAPs, label='mAP@[0.5:0.95]')
    plt.xlabel('Epoch')
    plt.ylabel('mAP')
